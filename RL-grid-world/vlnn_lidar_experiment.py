...
class VolumetricCognitiveBrain:
    ...
    def get_parameter_count(self):
        # Cada neurona: Centro(6) + Radio(1) + Q_vals(5) + Stats(1) = ~13 floats
        return len(self.neurons) * (6 + 1 + 5 + 1)
...
if __name__ == "__main__":
    ...
    print("--- 🧠 DUELO V3: LIDAR SENSORS (6D) ---")
    
    print("\n>>> Entrenando Motor Cognitivo (VLNN - Lidar)...")
    m_vlnn, t_vlnn = train("VLNN (Lidar)", env, map_manager)
    
    print("\n>>> Entrenando Red Neuronal (MLP - Lidar)...")
    ...
    # Win Rate
    plt.subplot(1, 2, 1)
    if len(m_vlnn['success']) > 30: plt.plot(smooth(m_vlnn['success']), label='VLNN (Lidar)', color='blue', linewidth=2)
    if len(m_mlp['success']) > 30: plt.plot(smooth(m_mlp['success']), label='MLP (Lidar)', color='magenta', linestyle='--')
    plt.title("Tasa de Éxito (LIDAR 6D)")
    ...
    plt.tight_layout()
    plt.savefig('vnn_lidar_experiment.png')
    print("\nResultados guardados en 'vnn_lidar_experiment.png'")
    ...
    # JSON OUTPUT
    results = {
        "experiment": "Real Cognitive VLNN vs Deep MLP (LIDAR 6D)",
        "metrics": {
            "VLNN": {
